		...
		<div id="wrapper">
			...
			<div id="profileUpdateBox">
				...
				<form id="updateForm" action="profileUpdate.action">
					...
					</select> <span id="msg_false"> 희망 포지션을 선택해주세요.</span> <br> <br> <span
						class="profileItem">사용 도구</span> <span style="font-size: 8pt;">(다중선택
						가능)</span> <span id="msg_false"> 사용 가능한 도구를 1개 이상 선택해주세요!</span> <br />
					<br />
					...
					<c:forEach var="tool" items="${toolList}" varStatus="loopStatus">
						...
						<c:choose>
							<c:when test="${tool.pinNo != null}">
            							┃<label>${tool.toolName} <input
									type="checkbox" id="tool${tool.toolNo}" name="toolCheckBox"
									value="${tool.toolNo}" checked="checked" />
								</label>
							</c:when>
							<c:otherwise>
						           		┃<label>${tool.toolName} <input
						...
						<c:forEach var="updateSubRegion" items="${updateSubRegionList}">
							<c:choose>
								<c:when test="${updateSubRegion.pinNo != null}">
									<option value="${updateSubRegion.subRegionNo }"
										selected="selected">${updateSubRegion.subRegionName }</option>
								</c:when>
								<c:otherwise>
